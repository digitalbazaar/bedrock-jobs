...
function init(callback) {
  ...
  async.waterfall([
    function(callback) {
      ...
      database.openCollections(['job'], callback);
    },
    function(callback) {
    ...
    function(callback) {
      database.getDistributedIdGenerator('job',
 ...
 */
api.getJob = function(id, callback) {
  async.waterfall([
    function(callback) {
      database.collections.job.findOne(
        {id: database.hash(id)}, {}, callback);
    },
    function(record, callback) {
      if(!record) {
        return callback(new BedrockError(
          'Job not found.', 'NotFound', {id: id}));
      }
...
function _getNextJobSchedule(job, options) {
  ...
  var intervalParts = job.schedule.split('/');
  ...
  if(intervalParts.length === 2) {
    // R[n]/duration
    interval = moment.interval(moment(), moment.interval(interval[1]));
  } else {
  ...
  async.until(function() {return done;}, function(loopCallback) {
    ...
    async.auto({
      getIdleJob: function(callback) {
        ...
        var query = bedrock.util.extend({}, baseQuery, {
      ...
      getExpiredJob: ['getIdleJob', function(callback, results) {
        ...
        var query = bedrock.util.extend({}, baseQuery, {permits: 0, $or: []});
      ...
      checkResult: ['runJob', function(callback, results) {
        ...
        var logFn = error ? logger.error : logger.verbose;
        logFn(
